...
export class CriarSalaComponent {
  ...
  sala: CriarSalaRequest = {
    ...
    senha: '',
    categoria: 'chat',
    qtdCapacidade: null  // null = ilimitado
  }
  ...
  onCategoriaChange() {
    ...
    } else if (this.noMinimoTres()) {
      if (this.sala.qtdCapacidade !== null && this.sala.qtdCapacidade < 3) {
        this.sala.qtdCapacidade = 3;
      }
    }
    // chat: null = ilimitado é válido, mantém o valor atual
  }
    ...
    this.service.criar(this.sala).subscribe({
      next: (response: any) => {
        this.router.navigate([`/salas/${this.username}/${this.sala.nome}`])
          .then(() => { this.successHandler.handleSuccess('Sala criada com sucesso!') });
      },
      error: (err: any) => {
